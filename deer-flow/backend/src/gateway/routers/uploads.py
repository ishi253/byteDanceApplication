...
async def upload_files(
    ...
    # Best-effort: kick off vector RAG ingestion in background (if configured).
    # Keeps upload latency low; the index should be ready by the next user message.
    if background_tasks is not None:
        try:
            from src.community.rag_milvus.tools import rag_ingest_uploads_vector_tool

            background_tasks.add_task(rag_ingest_uploads_vector_tool, thread_id)
        except Exception as e:
            logger.info(f"Skipping background RAG ingestion (vector): {e}")
