...
# Use pnpm if available, otherwise npm (so make dev works without pnpm)
if command -v pnpm >/dev/null 2>&1; then
    FRONTEND_PACKAGE_MGR="pnpm"
else
    FRONTEND_PACKAGE_MGR="npm"
fi
if $DEV_MODE; then
    FRONTEND_CMD="$FRONTEND_PACKAGE_MGR run dev"
else
    FRONTEND_CMD="env BETTER_AUTH_SECRET=$(python3 -c 'import secrets; print(secrets.token_hex(16))') $FRONTEND_PACKAGE_MGR run preview"
fi
...
if command -v nginx >/dev/null 2>&1; then
    mkdir -p logs/nginx-tmp/{client_body,proxy,fastcgi,uwsgi,scgi} 2>/dev/null || true
    echo "Starting Nginx reverse proxy..."
    if nginx -g 'daemon off;' -c "$REPO_ROOT/docker/nginx/nginx.local.conf" -p "$REPO_ROOT" > logs/nginx.log 2>&1 &
    then
        NGINX_PID=$!
        if ./scripts/wait-for-port.sh 2026 10 "Nginx"; then
            NGINX_STARTED=true
            echo "✓ Nginx started on localhost:2026"
        else
            kill "$NGINX_PID" 2>/dev/null || true
            echo "  ⚠ Nginx failed to bind; continuing without it."
        fi
    else
        echo "  ⚠ Nginx failed to start; continuing without it."
    fi
else
    echo "  ⚠ nginx not found; skipping reverse proxy. Use frontend and gateway URLs below."
fi
...
[ "$NGINX_STARTED" = true ] && echo "     - Nginx:     logs/nginx.log"
echo ""
